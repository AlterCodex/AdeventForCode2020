...
def is_unique_chars(sub_string):
    unique_dict = {}
    for i in sub_string:
...
def find_first_marker(stream_array, length):
    result = []
    for stream in stream_array:
        for j in range(len(stream) - length + 1):
            if is_unique_chars(stream[j:j + length]):
                result.append(str(j + length))
                break
    return result

def process(file_name,k):
    stream_array = []
    with open(file_name, 'r') as file:
        for line in file:
            stream_array.append(line.strip())
    return find_first_marker(stream_array,k)

files = {'test': 'test.txt', 'prod': 'input.txt'}

if __name__ == "__main__":
    mode= "prod"
    print(' '.join(process(files[mode],4)))
