...
Maybe you can learn something by looking at the value of the X register throughout execution.
 For now, consider the signal strength (the cycle number multiplied by the value of the X register) during the 20th
 cycle and every 40 cycles after that (that is, during the 20th, 60th, 100th, 140th, 180th, and 220th cycles).
...
class Instruction():
    base_cost = {'noop': 1, 'addx': 2}

    def __init__(self, *args):
        self.instruction = args[0]
        self.value = 0
        if self.instruction == 'addx':
            self.value = int(args[1])
        self.cost = Instruction.base_cost[self.instruction]
