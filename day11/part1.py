...
class Monkey(object):
    ...
    def __init__(self, monkey_id):
        self._items = []
        self.operation = ''
        self.decision_coef = 1
        self.left = None
        self.right = None
        self.id = monkey_id
        self.inspected = 0

    def turn(self, common_divisor, worry_reduce):
        self._items.reverse()
        while len(self._items) > 0:
            self.inspected += 1
            item = self._items.pop()
            self.inspect(item, common_divisor, worry_reduce)

    def inspect(self, item, common_divisor, worry_reduce):
        old = item
        new = eval(self.operation)
        new = new % common_divisor
        new = new // worry_reduce
        if new % self.decision_coef == 0:
            self.right.give(new)
        else:
            self.left.give(new)
    ...
    def set_decision(self, decision_coef):
    ...
    with open(file_name, 'r') as file:
        for line in file:
            ...
            if instructions[0] == "Monkey":
                monkey_id = int(instructions[1][:-1])
                if monkey_id not in monkeys.keys():
                    monkeys[monkey_id] = Monkey(monkey_id)
            elif instructions[0] == "Starting":
                items = clean_line.split(':')[1]
                for item in items.split(','):
                    monkeys[monkey_id].give(int(item))
            elif instructions[0] == "Operation:":
                monkeys[monkey_id].set_operation(' '.join(instructions[3:]))
            elif instructions[0] == "Test:":
                monkeys[monkey_id].set_decision(int(instructions[-1]))
            elif instructions[0] == "":
                pass
            elif instructions[1] == "true:":
                right_monkey = int(instructions[-1])
                if right_monkey not in monkeys.keys():
                    monkeys[right_monkey] = Monkey(right_monkey)
                monkeys[monkey_id].set_right(monkeys[right_monkey])
            elif instructions[1] == "false:":
                left_monkey = int(instructions[-1])
                if left_monkey not in monkeys.keys():
                    monkeys[left_monkey] = Monkey(left_monkey)
                monkeys[monkey_id].set_left(monkeys[left_monkey])
        return monkeys


def play(monkeys, rounds, worry_reduce):
    common_divisor = functools.reduce(lambda cd, x: cd * x, (monkeys[m].decision_coef for m in monkeys))
    for i in range(rounds):
        #print(f"round {i}")
        for j in range(len(monkeys)):
        #    print(f"monkey {j}")
            monkey = monkeys[j]
            monkey.turn(common_divisor, worry_reduce)
    return monkeys
...
def calculate(monkeys):
    ...
    for i in range(len(monkeys)):
        scores.append(monkeys[i].inspected)
    sort_score = sorted(scores)
    return sort_score[-1] * sort_score[-2]


if __name__ == "__main__":
    mode = "prod"
    monkeys = build(files[mode])
    monkeys = play(monkeys, 20, 3)
    monkeys_bussiness = calculate(monkeys)
    print(f'part1 {monkeys_bussiness}')

    monkeys = build(files[mode])
    monkeys = play(monkeys, 10000, 1)
    monkeys_bussiness = calculate(monkeys)
    print(f'part 2: {monkeys_bussiness}')
