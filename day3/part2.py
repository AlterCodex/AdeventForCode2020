...
"""
from day3.utils import char_to_int

files = {'test': 'test.txt', 'prod': 'input.txt'}


def find_repeated_character(lines, amount):
    result = {}
    i = 0
    for line in lines:
        for c in line:
            if i == 0:
                result[c] = [i]
            elif c in result and i not in result[c]:
                result[c].append(i)
                if len(result[c]) == amount:
                    return c
        i+=1


def calculate_the_sum_of_priorities(mode):
    priorities = 0
    lines = []
    with open(files[mode], "r") as file:
        for line in file:
            ...
            if len(lines) == 3:
                priorities += char_to_int(find_repeated_character(lines, 3))
                lines = []
    if lines:
        priorities += char_to_int(find_repeated_character(lines, len(lines)))

    print(priorities)

if __name__ == '__main__':
    calculate_the_sum_of_priorities('prod')