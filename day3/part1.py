...
the priority of the item type that appears in both compartments
...
def find_doubled_characters(rucksack):
    half = len(str(rucksack).strip()) // 2
    first = rucksack[:half]
...
def calculate_the_sum_of_priorities(mode):
    ...
    with open(files[mode], "r") as file:
        for line in file:
            priorities += char_to_int(find_doubled_characters(line))
    print(priorities)
